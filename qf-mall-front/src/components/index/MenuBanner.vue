...
// 定义响应式数据
const bannerMenuFlag = ref(false)
let menuTimer: number | undefined
const listInfoData = ref<ListItem[]>([])
...
// 计算属性
const menuListMatch = computed(()=>{
    const matchData: ListItem[][] = []
    if (listInfoData.value && listInfoData.value.length) {
        for (let i = 0; i < listInfoData.value.length; i += 6) {
            matchData.push(listInfoData.value.slice(i, i + 6));
        }
    }
    return matchData
})

const bannerMenuHide = () => {
    menuTimer = window.setTimeout(() => {
        bannerMenuFlag.value = false
    }, 300)
}

const bannerMenuShow = (type? : string) => {
    if (type) {
        listInfoData.value = menus.value.find(menu => menu.type === type)?.listInfoData || []
    }
    bannerMenuFlag.value = true
